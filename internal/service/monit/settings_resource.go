...
func (r *settingsResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_monit_settings"
}
...
func (r *settingsResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	resp.Diagnostics.AddError(
		...
		"This resource manages existing upstream Monit configuration that cannot be created or destroyed.\n\n"+
			"To manage this resource, you must import it first:\n"+
			"  terraform import opnsense_monit_settings.<name> monit_settings\n\n"+
...
func (r *settingsResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	result, err := r.client.Monit().SettingsGet(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read monit settings, got error: %s", err))
		return
	}
	...
	tflog.Trace(ctx, "read monit settings resource")
...
func (r *settingsResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	_, err = r.client.Monit().SettingsReconfigure(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to reconfigure monit settings, got error: %s", err))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read updated monit settings, got error: %s", err))
		return
	}
	...
	tflog.Trace(ctx, "updated monit settings resource")
...
func (r *settingsResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Warn(ctx,
		"Singleton resource removed from Terraform state. "+
			"The upstream Monit configuration remains unchanged and will not be deleted.")

	resp.Diagnostics.AddWarning(
		"Singleton Resource Removed From State Only",
		"This resource has been removed from Terraform state, but the upstream "+
			"Monit configuration has NOT been deleted or modified. The settings "+
...
func (r *settingsResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	if req.ID != "monit_settings" {
		resp.Diagnostics.AddError(
			"Invalid Import ID",
			"This is a singleton resource and must be imported using the ID 'monit_settings'.\n\n"+
				"Usage:\n"+
				"  terraform import opnsense_monit_settings.<name> monit_settings\n\n"+
	...
	tflog.Info(ctx, "imported monit settings resource", map[string]any{"id": req.ID})
}
